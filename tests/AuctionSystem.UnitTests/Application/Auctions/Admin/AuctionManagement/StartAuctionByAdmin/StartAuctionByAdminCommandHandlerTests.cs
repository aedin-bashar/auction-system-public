...
public class StartAuctionByAdminCommandHandlerTests
{
    [Fact]
    public async Task Handle_WithActiveAdminAndDraftAuction_StartsAuctionAndPersists()
    {
        var admin = User.Register("admin@example.com", "Admin User", UserRole.Admin);
        var auction = CreateDraftAuction();
        var command = new AuctionSystem.Application.Auctions.Admin.AuctionManagement.StartAuctionByAdmin.StartAuctionByAdminCommand(admin.Id, auction.Id);
        ...
        var handler = new AuctionSystem.Application.Auctions.Admin.AuctionManagement.StartAuctionByAdmin.StartAuctionByAdminCommandHandler(
            users.Object,
            auctions.Object,
            unitOfWork.Object);
    ...
    public async Task Handle_WhenRequesterIsNotActiveAdmin_ThrowsUnauthorizedAccessException()
    {
        ...
        var command = new AuctionSystem.Application.Auctions.Admin.AuctionManagement.StartAuctionByAdmin.StartAuctionByAdminCommand(bidder.Id, Guid.NewGuid());
    ...
    {
        var admin = User.Register("admin@example.com", "Admin User", UserRole.Admin);
        var command = new AuctionSystem.Application.Auctions.Admin.AuctionManagement.StartAuctionByAdmin.StartAuctionByAdminCommand(admin.Id, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_WhenAuctionIsNotDraft_ThrowsInvalidOperationException()
    {
        var admin = User.Register("admin@example.com", "Admin User", UserRole.Admin);
        var auction = CreateDraftAuction();
        auction.Start();
    ...
    private static Auction CreateDraftAuction() => Auction.Create(
        Guid.NewGuid(),
        "Draft Auction",
